...
## Ch 7 - Basics of Classes and Objects

### 7.1 - Using a class to encapsulate data & processing
...
Another way of considering a class is as a group of closely-related functions working wtih common data. We call these methods for working with the data. A class definition should contain the smallest collection of methods needed for working with the object's data.

A software object can be viewed as analagous to a thing - a noun. The behaviors of the class can be viewed as verbs. This identification with nouns and verbs gives us a hint as to how we can proceed to design classes to work effectively.

Example: creating a dice game, using a pair of dice for game simulation.
1. Write down a description of what an instance of the class does. These are problem statements. It's essential to focus on single-verb sentences, with a focus on only the nouns and verbs. Examples:
    - The game of Craps has two standard dice.
    - Each die has six faces, with point values from one to six.
    - Dice are rolled by the player. While writers and editors prefer the active voice, "A player rolls the dice," the dice are often acted upon by other objects, making the passive voice slightly more useful.
    - The total of the dice changes the state of the Craps game. Those rules are separate from the dice.
    - If the two dice match, the number was described as being rolled "the hard way". If the two dice dont match, the roll was described as being made "the easy way".
2. Identify all of the nouns in the sentences. With this example, the nouns include dice, faces, point values, and a player. The nouns identify different classes of objects that may be collaborators, like a player or a game. Nouns may also identify attributes of objects, like face or point value.
3. Identify all the verbs of the sentence. Verbs become methods of the class. In this example, verbs include roll and match.
...
class Dice:
    ...
    def easyway(self) -> bool:
        return self.faces[0] != self.faces[1]
```
The core idea here is to use ordinary rules of grammar - nouns, verbs, and adjectives - as a way to identify basic features of a class.
