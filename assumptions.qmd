...
```
Visual inspection of the QQ plots and histograms should be enough to help you conclude whether the normality assumption has been violated.
...
### Kolmogorov-Smirnov
 ...
 - Measures a calculated test statistic against a table-based critical value:
    - If test statistic > critical value, null hypthesis that the given distribution is normally distribution can be _rejected_.
    - Can assess if the `p-value` is low enough to be significant.
    - Test statistic is calculated as the absolute value of the maximum distance between all data points in a given distribution against the `cdf`(cumulative density function).
 - One-sample goodness-of-fit test: performs analysis against a benchmark `CDF`(cumulative density function).
 - Running the kstest` function in the `scipy.stats`, Using `stats.norm.cdf` performs the one-sample version.
 - Two-sample version tests against a specified distribution to determine whether the two distributions match.
 ...
 - Used to test the null hypothesis that a given sample distribution is normally distributed.
 ...
 - One of three most commonly used tests, the other two are:
   - **`Anderson-Darling`** test
     ...
     - More likely to identify non-conformity to normality if data is heay-tailed w/ extreme outliers.
...
- Alternatively, may be better to lower the confidence level for the test being applied.
