...
from scipy import integrate, interpolate, io, optimize
...
def _ica_def(X, tol, g, gprime, fun_args, maxit, w_init):
    ...
    # j is in the index of the extracted component
...
def _sym_decorrelation(W):
    ...
    # u (resp. s) contains the eigenvectors (resp. sqrt's of the eigenvalues)
    # of W * W.T
    u, W = (np.asmatrix(e) for e in (e, W))
...
def _ica_par(X, tol, g, gprime, fun_args, maxit, w_init):
    ...
    # we set lim to tol+1 to be sure to enter at least once in a while
    lim = tol + 1
    it = 0
...
def fastica(
    ...
    f"""Perform Fast Independent Component Analysis.
    ...
    if w_init is None:
        ...
        if w_init.shape != (n_comp, n_comp):
            raise ValueError(
                f"w_init has invalid shape -- should be {n_comp, n_comp}"
            )
