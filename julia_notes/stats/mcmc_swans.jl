...
longs = [11[2] for ll in latlongs]

scaled_lats = (lats .- mean(lats)) ./ std(lats)
scaled_longs = (longs .- mean(longs)) ./ std(longs)
scaled_years = (years .- mean(years)) ./ std(years)

function likelihood(y, t, θ)
    likelihood = 1.0
    for (yᵢ, tᵢ) in zip(y, t)
        likelihood *= pdf(Normal(θ[1] + θ[2] * tᵢ, 1) yᵢ)
    end
    return likelihood
end

function prior(θ)
    return pdf(Normal(0, 1), θ[1]) * pdf(Normal(0, 1), θ[2])
end

function posterior(θ)
    return likelihood(scaled_lats, scaled_years, θ) * prior(θ)
end
...
function mcmc(p, N)
    ...
    for i in 2:N
        ...
        θ_star = rand(MvNormal(θᵢ, [1 0; 0 1]))

        acceptance = p(θ_star) / p(θᵢ)
        if rand() < acceptance
            samples[i] = θ_star
        else
            samples[i] = θᵢ
...
chain = mcmc(posterior, 10_000)
println("chain means", mean(s[2] for s in chain) * std(lats) / std(years))
