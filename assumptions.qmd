...
plt.show()
```
Visual inspection of the QQ plots and histograms should be enough to help you
conclude whether the normality assumption has been violated.
...
- Measures a calculated test statistic against a table-based critical value:
  - If test statistic > critical value, null hypthesis that the given
    distribution is normally distribution can be *rejected*.
  - Can assess if the `p-value` is low enough to be significant.
  - Test statistic is calculated as the absolute value of the maximum distance
    between all data points in a given distribution against the `cdf`(cumulative
    density function).
- One-sample goodness-of-fit test: performs analysis against a benchmark
  `CDF`(cumulative density function).
- Running the kstest`function in the`scipy.stats`, Using`stats.norm.cdf\`
  performs the one-sample version.
- Two-sample version tests against a specified distribution to determine whether
  the two distributions match.
  - Must be provided as a numpy array instead of stats.norm.cdf.
- This test requires data to be centered at 0 with unit variance.
- Used to test the null hypothesis that a given sample distribution is normally
  distributed.
- Focuses on the centrality of the data.
- One of three most commonly used tests, the other two are:
  - **`Anderson-Darling`** test
    - Focuses more on the tails of the data than the center
    - More likely to identify non-conformity to normality if data is heay-tailed
      w/ extreme outliers.
    - Like Kolmogorov-Smirnov it performs well on large sample sizes
  - **`Shapiro-Wilk`** test
    - More general than both `AD` and `KS` tests
    - More robust to small sample sizes
      - Useful in an automated pipeline
- Alternatively, may be better to lower the confidence level for the test being
  applied.
...
print(stats.kstest(normally_distributed, stats.norm.cdf))
print(stats.kstest(np.exp(normally_distributed), stats.norm.cdf))
```

Showing a distribution of 1,000 samples with a
$\mu = 100  \text{ and } \sigma = 2$ It needs to be centered to a mean of 0 with
...
# the data is centered and scaled, check it with the Kolmogorov-Smirnov test.
print(stats.kstest(normally_distributed_scaled), stats.norm.cdf)
```
